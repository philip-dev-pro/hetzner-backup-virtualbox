...
* **Problem:** The restored virtual machine took several minutes to boot and frequently appeared completely stuck during startup. This happened because `cloud-init` was continuously searching and waiting for network responses from Hetzner's cloud metadata services, which do not exist outside of Hetzner. Additionally, hard power-offs during testing caused unclean shutdowns, leading to journal corruption and filesystem check (`fsck`) delays at boot.
* **Solution:** We disabled cloud-init by running `touch /etc/cloud/cloud-init.disabled` inside the server. This completely removed the timeout loops, prevented the machine from hanging, and reduced boot time down to just a few seconds. Furthermore, we booted into GRUB Recovery Mode to run `fsck` to cleanly repair the filesystem, and made sure to use VirtualBox's graceful shutdown option instead of forced hard power-offs.
